...
tensorflow::Status EmbeddingWrapper<TypeKey, TypeFP>::fprop(
    const tensorflow::Tensor* sparse_indices, const tensorflow::Tensor* values,
    const tensorflow::Tensor* dense_shape, const std::string& embedding_name,
    const bool is_training, tensorflow::Tensor* const forward_result, const bool on_gpu) {
  tensorflow::Status status;

  /*get embedding params for this instance*/
  std::shared_ptr<EmbeddingParams> params = get_embedding_params(embedding_name);
  if (!params)
    return tensorflow::errors::NotFound(__FILE__, ": ", __LINE__,
                                        " Not found embedding params for ", embedding_name);
  ...
  if (std::is_same<TypeFP, float>::value) {
    std::dynamic_pointer_cast<IEmbeddingForTensorFlowPlugin>(embedding)->get_forward_results_tf(
        is_training, on_gpu, reinterpret_cast<void*>(forward_result->flat<float>().data()));
  } else if (std::is_same<TypeFP, __half>::value) {
    std::dynamic_pointer_cast<IEmbeddingForTensorFlowPlugin>(embedding)->get_forward_results_tf(
        is_training, on_gpu, reinterpret_cast<void*>(forward_result->flat<Eigen::half>().data()));
  } else {
