...
tensorflow::Status EmbeddingWrapper<TypeKey, TypeFP>::fprop_v2(
    const tensorflow::OpInputList& row_offsets, const tensorflow::OpInputList& value_tensors,
  ...
  for (size_t dev_id = 0; dev_id < gpu_count; ++dev_id) {
    ...
    WRAPPER_CUDA_CHECK(cudaMemcpyAsync(
        space->row_offsets_tensors_[dev_id].get_ptr(), row_offset_flat.data(),
        space->row_offsets_tensors_[dev_id].get_size_in_bytes(), cudaMemcpyDeviceToDevice,
        resource_manager_->get_local_gpu(dev_id)->get_stream()));

    auto value_tensor_flat = value_tensors[dev_id].flat<TypeKey>();
    WRAPPER_CUDA_CHECK(
        cudaMemcpyAsync(space->value_tensors_[dev_id].get_ptr(), value_tensor_flat.data(),
