...
tensorflow::Status EmbeddingWrapper<TypeKey, TypeFP>::bprop(const std::string& embedding_name,
  ...
  if (std::is_same<TypeFP, float>::value) {
    WRAPPER_CUDA_CHECK(
        std::dynamic_pointer_cast<IEmbeddingForTensorFlowPlugin>(embedding)->update_top_gradients(
            on_gpu, reinterpret_cast<const void*>(top_gradients->flat<float>().data())));
  } else if (std::is_same<TypeFP, __half>::value) {
    WRAPPER_CUDA_CHECK(
        std::dynamic_pointer_cast<IEmbeddingForTensorFlowPlugin>(embedding)->update_top_gradients(
            on_gpu, reinterpret_cast<const void*>(top_gradients->flat<Eigen::half>().data())));
  } else {
...
template tensorflow::Status EmbeddingWrapper<long long, float>::bprop(
    const std::string& embedding_name, const tensorflow::Tensor* top_gradients, const bool on_gpu);
