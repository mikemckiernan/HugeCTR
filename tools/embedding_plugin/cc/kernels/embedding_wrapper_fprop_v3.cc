...
tensorflow::Status EmbeddingWrapper<TypeKey, TypeFP>::fprop_v3(
    const tensorflow::Tensor* row_offsets, const tensorflow::Tensor* value_tensors,
    const tensorflow::Tensor* nnz_array, const std::string& embedding_name, const bool is_training,
  ...
  if (row_offsets->dims() != 2)
    return tensorflow::errors::Aborted(
        __FILE__, ":", __LINE__, " row_offsets rank should be 2, but got ", row_offsets->dims());
  if (row_offsets->dim_size(0) != gpu_count_long)
    return tensorflow::errors::Aborted(__FILE__, ":", __LINE__,
                                       " row_offsets shape[0] should be equal to gpu_count.");
  if (value_tensors->dims() != 2)
    return tensorflow::errors::Aborted(__FILE__, ":", __LINE__,
                                       " value_tensors rank should be 2, but got ",
                                       value_tensors->dims());
  if (value_tensors->dim_size(0) != gpu_count_long)
    return tensorflow::errors::Aborted(__FILE__, ":", __LINE__,
                                       " value_tensors shape[0] should be equal to gpu_count.");
  if (nnz_array->dims() != 1 || nnz_array->dim_size(0) != gpu_count_long)
    return tensorflow::errors::Aborted(
        __FILE__, ":", __LINE__, " nnz_array rank should be 1 and shape[0] should be gpu_count.");
  ...
  if (!space)
    return tensorflow::errors::NotFound(__FILE__, ": ", __LINE__, ", Did not find ",
  ...
  WRAPPER_CUDA_CHECK(cudaMemcpy(host_nnz_array.get(), nnz_array_flat.data(),
                                sizeof(long long) * nnz_array_flat.size(), cudaMemcpyDeviceToHost));
  ...
  for (size_t dev_id = 0; dev_id < gpu_count; ++dev_id) {
    ...
    WRAPPER_CUDA_CHECK(
        cudaMemcpyAsync(space->value_tensors_[dev_id].get_ptr(),
                        value_tensors_flat.data() + value_tensors->dim_size(1) * dev_id,
                        space->value_tensors_[dev_id].get_size_in_bytes(), cudaMemcpyDeviceToDevice,
                        resource_manager_->get_local_gpu(dev_id)->get_stream()));
    /*write nnz buffer*/
  ...
  for (size_t dev_id = 0; dev_id < gpu_count; ++dev_id) {
    ...
    WRAPPER_CUDA_CHECK(
        cudaStreamSynchronize(resource_manager_->get_local_gpu(dev_id)->get_stream()));
  }

  /*forward propagation*/
  std::shared_ptr<IEmbedding> embedding = get_embedding(embedding_name);
  if (!embedding)
    return tensorflow::errors::NotFound(__FILE__, ":", __LINE__, " ", "Not found ", embedding_name);
  embedding->forward(is_training);

  /*get forward results*/
  if (std::is_same<TypeFP, float>::value) {
    std::dynamic_pointer_cast<IEmbeddingForTensorFlowPlugin>(embedding)->get_forward_results_tf(
        is_training, true, reinterpret_cast<void*>(forward_result->flat<float>().data()));
  } else if (std::is_same<TypeFP, __half>::value) {
    std::dynamic_pointer_cast<IEmbeddingForTensorFlowPlugin>(embedding)->get_forward_results_tf(
        is_training, true, reinterpret_cast<void*>(forward_result->flat<Eigen::half>().data()));
  } else {
    return tensorflow::errors::Unimplemented(__FILE__, ":", __LINE__,
                                             " TypeFP should be {float, __half}.");
  }
