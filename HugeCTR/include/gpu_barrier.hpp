...
namespace HugeCTR
{
  class GPUBarrier
  {
    public:
      ...
      GPUBarrier(const std::shared_ptr<const ResourceManager>& resource_manager);
      ~GPUBarrier();

      void sync_all_gpus(const cudaStream_t* streams);
      void sync_all_gpus(const cudaStream_t stream, size_t device_id);
      void sync_all_gpus_report_host(size_t** d_report_count, 
    ...
    private:
      ...
      size_t num_gpus_;
      const std::shared_ptr<const ResourceManager> resource_manager_;
  };
}
