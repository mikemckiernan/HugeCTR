...
class RawSource : public Source {
 ...
 public:
  RawSource(std::shared_ptr<RawOffsetList> raw_offset_list, int worker_id)
    ...
    buffer_ = static_cast<char*>(aligned_alloc(alignment_bytes_,
                                                batch_size_ * stride_ + alignment_bytes_));
  }
  ...
  char* get_ptr() { 
    ...
    size_t raw_end_offset = ( (req_end_offset + alignment_bytes_ - 1) / alignment_bytes_) * alignment_bytes_;
