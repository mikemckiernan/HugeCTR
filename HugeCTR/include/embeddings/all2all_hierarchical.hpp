...
namespace HugeCTR {
  ...
  int InterNodeHierarchicalAlltoAll<TypeEmbeddingComp>::create_nccl_comms()
  {
    ...
    for (size_t g = 0; g < local_gpu_count_; g++) {
      ...
      if (pid_ == 0) { CK_NCCL_THROW_(ncclGetUniqueId(&inter_id)); }
  ...
  template <typename TypeEmbeddingComp>
  size_t InterNodeHierarchicalAlltoAll<TypeEmbeddingComp>::get_slot_num_per_node(size_t my_node ) const 
  {
  ...
  template<> inline ncclDataType_t 
  InterNodeHierarchicalAlltoAll<float>::get_nccl_type()  const { return ncclFloat32; }
  
  template<> inline ncclDataType_t 
  InterNodeHierarchicalAlltoAll<__half>::get_nccl_type() const { return ncclFloat16; }

  // Initialization
  template <typename TypeEmbeddingComp>
  int InterNodeHierarchicalAlltoAll<TypeEmbeddingComp>::init(
      const ResourceManager* resource_manager,
      size_t slot_num,
      size_t train_batch_size,
      size_t eval_batch_size,
      ...
      Tensors2<TypeEmbeddingComp>& output)
  {
    ...
    for (size_t g = 0; g < local_gpu_count_; g++) {
      ...
      for (int p = 0; p < num_procs_; p++) {
        PROFILE_RECORD("inter_node_hier_a2a.fprop.start", resource_manager_->get_local_gpu(g)->get_stream(), false);
        ...
        CK_NCCL_THROW_(ncclRecv(
              ...
              nccl_dtype_, p, inter_comms_[g],
              resource_manager_->get_local_gpu(g)->get_stream()));
        PROFILE_RECORD("inter_node_hier_a2a.fprop.stop", resource_manager_->get_local_gpu(g)->get_stream(), false);
      }
      CK_NCCL_THROW_(ncclGroupEnd());
    }
    return 0;
  ...
  int InterNodeHierarchicalAlltoAll<TypeEmbeddingComp>::bprop(
      Tensors2<TypeEmbeddingComp>& bprop_input,
      Tensors2<TypeEmbeddingComp>& bprop_output)
  {
    ...
    for (size_t g = 0; g < local_gpu_count_; g++) {
      ...
      CK_NCCL_THROW_(ncclGroupStart());
      for (int p = 0; p < num_procs_; p++) {
        PROFILE_RECORD("inter_node_hier_a2a.bprop.start", resource_manager_->get_local_gpu(g)->get_stream(), false);
        ...
        CK_NCCL_THROW_(ncclRecv(
              bprop_output[g].get_ptr() + vars.inter_send_offsets_[p],
              vars.inter_send_counts_[p],
              nccl_dtype_, p, inter_comms_[g],
              resource_manager_->get_local_gpu(g)->get_stream()));
        PROFILE_RECORD("inter_node_hier_a2a.bprop.stop", resource_manager_->get_local_gpu(g)->get_stream(), false);
      }
      CK_NCCL_THROW_(ncclGroupEnd());
    }
    return 0;
