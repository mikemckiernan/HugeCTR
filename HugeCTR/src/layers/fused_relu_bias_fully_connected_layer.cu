...
// #endif
  ...
  PROFILE_RECORD("fused_relu_bias_fully_connected.fprop.cublasLtMatmul.start", get_gpu().get_stream());
  CK_CUBLAS_THROW_(cublasLtMatmul(get_gpu().get_cublaslt_handle(),
              ...
              get_gpu().get_stream()));
  PROFILE_RECORD("fused_relu_bias_fully_connected.fprop.cublasLtMatmul.stop", get_gpu().get_stream());
    if(pos_ == TAIL || pos_ == ISOLATED)
    {
        size_t len = top_tensor_fprop_.get_num_elements();
        // Curretly inline profiler has issue with these kind of selective executed kernel. Skip for now.
        // PROFILE_RECORD("fused_relu_bias_fully_connected.fprop.memcopy_to_top.start", get_gpu().get_stream(), get_device_id());
        CK_CUDA_THROW_(cudaMemcpyAsync(top_bprop, top_fprop,
            len*sizeof(__half), cudaMemcpyDeviceToDevice, get_gpu().get_stream()));
        // PROFILE_RECORD("fused_relu_bias_fully_connected.fprop.memcopy_to_top.stop", get_gpu().get_stream(), get_device_id());
    }
...
// #endif
  ...
  PROFILE_RECORD("fused_relu_bias_fully_connected.bprop.initialize_array.start", get_gpu().get_stream());
  initialize_array<<<(n - 1) / 1024 + 1, 1024, 0, get_gpu().get_stream()>>>(bias_grad_float, n,
                                                                            0.0f);
  PROFILE_RECORD("fused_relu_bias_fully_connected.bprop.initialize_array.stop", get_gpu().get_stream());

  dim3 blocks(n / 64, m / 32);

  PROFILE_RECORD("fused_relu_bias_fully_connected.bprop.reverse_add_bias_and_re_kernel.start", get_gpu().get_stream());
  reverse_add_bias_and_re_kernel<32>
      <<<blocks, 512, 0, get_gpu().get_stream()>>>(bias_grad_float, middle, top, n / 2);
  PROFILE_RECORD("fused_relu_bias_fully_connected.bprop.reverse_add_bias_and_re_kernel.stop", get_gpu().get_stream());

  PROFILE_RECORD("fused_relu_bias_fully_connected.bprop.convert_array.start", get_gpu().get_stream());
  convert_array<<<(n - 1) / 1024 + 1, 1024, 0, get_gpu().get_stream()>>>(bias_grad, bias_grad_float,
                                                                         n);
  PROFILE_RECORD("fused_relu_bias_fully_connected.bprop.convert_array.stop", get_gpu().get_stream());

  PROFILE_RECORD("fused_relu_bias_fully_connected.bprop.cublasGemmEx_1.start", get_gpu().get_stream());
  ...
  PROFILE_RECORD("fused_relu_bias_fully_connected.bprop.cublasGemmEx_1.stop", get_gpu().get_stream());

  PROFILE_RECORD("fused_relu_bias_fully_connected.bprop.cublasGemmEx_2.start", get_gpu().get_stream());
  ...
  PROFILE_RECORD("fused_relu_bias_fully_connected.bprop.cublasGemmEx_2.stop", get_gpu().get_stream());
