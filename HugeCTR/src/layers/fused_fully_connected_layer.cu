...
void FusedFullyConnectedLayer::fprop(bool is_train) {
  ...
  PROFILE_RECORD("fused_fully_connected.fprop.add_bias_and_re_kernel.start", get_gpu().get_stream());
...
void FusedFullyConnectedLayer::bprop() {
  ...
  PROFILE_RECORD("fused_fully_connected.bprop.reverse_add_bias_and_re_kernel.start", get_gpu().get_stream());
  reverse_add_bias_and_re_kernel<32>
      <<<blocks, 512, 0, get_gpu().get_stream()>>>(bias_grad_float, middle, top, n / 2);
  PROFILE_RECORD("fused_fully_connected.bprop.reverse_add_bias_and_re_kernel.stop", get_gpu().get_stream());
