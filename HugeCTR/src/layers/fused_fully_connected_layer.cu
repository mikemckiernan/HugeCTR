...
void FusedFullyConnectedLayer::fprop(bool is_train) {
  ...
  PROFILE_RECORD("fused_fully_connected.fprop.start", get_gpu().get_stream(), get_device_id());
  ...
  PROFILE_RECORD("fused_fully_connected.fprop.cublasGemmEx.start", get_gpu().get_stream(), get_device_id());
  ...
  PROFILE_RECORD("fused_fully_connected.fprop.cublasGemmEx.stop", get_gpu().get_stream(), get_device_id());

  const size_t max_threads = 1024;
  const size_t blocks = m;
  const size_t threads = min(n / 2, max_threads);

  PROFILE_RECORD("fused_fully_connected.fprop.add_bias_and_re_kernel.start", get_gpu().get_stream(), get_device_id());
  add_bias_and_re_kernel<<<blocks, threads, 0, get_gpu().get_stream()>>>(top, middle, bias, n / 2,
                                                                         n / 2);
  PROFILE_RECORD("fused_fully_connected.fprop.add_bias_and_re_kernel.stop", get_gpu().get_stream(), get_device_id());
  ...
  PROFILE_RECORD("fused_fully_connected.fprop.stop", get_gpu().get_stream(), get_device_id());
}

void FusedFullyConnectedLayer::bprop() {
  CudaDeviceContext context(get_device_id());
  PROFILE_RECORD("fused_fully_connected.bprop.start", get_gpu().get_stream(), get_device_id());
  ...
  PROFILE_RECORD("fused_fully_connected.bprop.initialize_array.start", get_gpu().get_stream(), get_device_id());
  initialize_array<<<(n - 1) / 1024 + 1, 1024, 0, get_gpu().get_stream()>>>(bias_grad_float, n,
                                                                            0.0f);
  PROFILE_RECORD("fused_fully_connected.bprop.initialize_array.stop", get_gpu().get_stream(), get_device_id());

  dim3 blocks(n / 64, m / 32);
  PROFILE_RECORD("fused_fully_connected.bprop.reverse_add_bias_and_re_kernel.start", get_gpu().get_stream(), get_device_id());
  reverse_add_bias_and_re_kernel<32>
      <<<blocks, 512, 0, get_gpu().get_stream()>>>(bias_grad_float, middle, top, n / 2);
  PROFILE_RECORD("fused_fully_connected.bprop.reverse_add_bias_and_re_kernel.stop", get_gpu().get_stream(), get_device_id());

  PROFILE_RECORD("fused_fully_connected.bprop.convert_array.start", get_gpu().get_stream(), get_device_id());
  convert_array<<<(n - 1) / 1024 + 1, 1024, 0, get_gpu().get_stream()>>>(bias_grad, bias_grad_float,
                                                                         n);
  PROFILE_RECORD("fused_fully_connected.bprop.convert_array.stop", get_gpu().get_stream(), get_device_id());

  PROFILE_RECORD("fused_fully_connected.bprop.cublasGemmEx_1.start", get_gpu().get_stream(), get_device_id());
  ...
  PROFILE_RECORD("fused_fully_connected.bprop.cublasGemmEx_1.stop", get_gpu().get_stream(), get_device_id());

  PROFILE_RECORD("fused_fully_connected.bprop.cublasGemmEx_2.start", get_gpu().get_stream(), get_device_id());
  ...
  PROFILE_RECORD("fused_fully_connected.bprop.cublasGemmEx_2.stop", get_gpu().get_stream(), get_device_id());

  PROFILE_RECORD("fused_fully_connected.bprop.stop", get_gpu().get_stream(), get_device_id());
