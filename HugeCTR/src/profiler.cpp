    ...
    for(int i = 0; i < int(unit_test_labels_.size()); i++) {
      ...
      if (event_type == "start") {
        ...
        if (event_idx >= 0) {
          auto event = events_[event_idx];
          event->end_index = i;
          auto it = std::find(streams.begin(), streams.end(), static_cast<GPUEvent*>(event.get())->stream);
          ...
          CK_CUDA_THROW_(cudaEventElapsedTime(&measured_time_ms, unit_test_events_[event->start_index], unit_test_events_[i]));
          CK_CUDA_THROW_(cudaEventElapsedTime(&start_to_event_start_ms, stream_first_points[stream_index], unit_test_events_[event->start_index]));
