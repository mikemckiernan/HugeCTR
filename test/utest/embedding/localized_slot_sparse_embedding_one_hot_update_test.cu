...
public:
    ...
    void init_data(const size_t num_samples, const size_t max_vocabulary_size, const size_t embedding_vec_size) {
...
template <typename TypeEmbeddingComp>
void update_test(
    const std::vector<size_t> &value_index, size_t max_vocabulary_size,
    size_t embedding_vec_size,
    const std::vector<TypeEmbeddingComp> &wgrad
  ) {
    ...
    for (auto const& pair : ref_categorize) {
        ...
        if (!category_equal) {
            std::cout << "Fail : the weights of category " << category << " are wrongly computed." << std::endl;
    ...
    std::cout << "number of correct elements : " << count_all - count_neq
              << " out of  " << count_all 
