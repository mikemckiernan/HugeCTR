...
void HybridEmbeddingInputGenerator<dtype>::generate_uniform_rand_table_sizes() {
  assert(config_.num_tables > 0);
  assert((size_t)config_.num_categories >= config_.num_tables);
...
void HybridEmbeddingInputGenerator<dtype>::create_probability_distribution() {
  ...
  for (size_t embedding = 0; embedding < num_embeddings; ++embedding) {
    ...
    if (embedding_size < 30) {
      ...
      // TODO: shuffle categories so that the probability is NOT monotonic
...
void HybridEmbeddingInputGenerator<dtype>::generate_categories(dtype* data, size_t batch_size,
                                                               bool normalized) {
  const size_t num_embeddings = config_.num_tables;
...
HybridEmbeddingInputGenerator<dtype>::HybridEmbeddingInputGenerator(
    HybridEmbeddingConfig<dtype> config_, size_t seed)
    : config_(config_), seed_(seed), gen_(seed) {
...
template <typename dtype>
std::vector<dtype> HybridEmbeddingInputGenerator<dtype>::generate_categorical_input_per_feature(
    size_t batch_size) {
...
template <typename dtype>
void HybridEmbeddingInputGenerator<dtype>::generate_categorical_input_per_feature(
