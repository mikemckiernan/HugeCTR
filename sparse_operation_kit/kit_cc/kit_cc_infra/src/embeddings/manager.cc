...
void EmbeddingManager::init(const size_t global_replica_id, const size_t global_batch_size) {
    ...
    auto helper = [this, &global_batch_size]() {
        ...
        if (global_batch_size < resource_mgr_->get_global_gpu_count())
            throw std::runtime_error(ErrorBase + "Too small global_batch_size, "
                                    "which is less than global_gpu_count");
        if (global_batch_size % resource_mgr_->get_global_gpu_count() != 0)
            throw std::runtime_error(ErrorBase + "global_batch_size is not divisible by global_gpu_count.");
...
void EmbeddingManager::create_embedding(const std::shared_ptr<ParamInterface>& param,
                                        const std::string input_dispatcher,
                                        const std::vector<std::string> input_dispatcher_subsequent_ops,
                                        const std::string embedding_lookuper,
                                        const std::string output_dispatcher,
                                        const std::vector<std::string> output_dispatcher_subsequent_ops,
...
std::unordered_map<std::string, std::shared_ptr<UpdatePreparer>> EmbeddingManager::create_preparers_for_Adam() {
    ...
    for (const auto& embedding : embeddings_) {
        ...
        auto preparer = GradUpdatePreparer::create(global_batch_size, max_feature_num,
                                                   max_vocabulary_size_per_gpu,
                                                   buffers_, host_buffers_, 
                                                   resource_mgr_);
