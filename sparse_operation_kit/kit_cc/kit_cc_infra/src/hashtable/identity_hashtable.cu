...
template <typename KeyType, typename ValType>
std::shared_ptr<IdentityHashTable<KeyType, ValType>> 
IdentityHashTable<KeyType, ValType>::create(const size_t capacity) {
    return std::shared_ptr<IdentityHashTable<KeyType, ValType>>(new IdentityHashTable(capacity));
}

template <typename KeyType, typename ValType>
size_t IdentityHashTable<KeyType, ValType>::get_and_add_value_head(size_t counter_add, cudaStream_t stream) {
...
template <typename KeyType, typename ValType>
void IdentityHashTable<KeyType, ValType>::get_insert(const void *d_keys, void *d_vals, 
                                                     size_t len, cudaStream_t stream) {
    return this->get(d_keys, d_vals, len, stream);
}

template <typename KeyType, typename ValType>
void IdentityHashTable<KeyType, ValType>::insert(const void *d_keys, const void *d_vals, 
                                                 size_t len, cudaStream_t stream) {
    // do nothing.
    return;
}
...
template <typename KeyType, typename ValType>
void IdentityHashTable<KeyType, ValType>::dump(void* d_key, void* d_val, 
