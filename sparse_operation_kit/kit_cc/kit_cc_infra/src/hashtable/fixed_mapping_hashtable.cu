...
__global__ void divisive_kernel(const KeyType *d_keys, ValType *d_vals, 
                                const size_t len, const ValType interval,
...
template <typename KeyType, typename ValType>
void Divisive<KeyType, ValType>::operator()(const void *d_keys, void *d_vals, 
                                            const size_t len, cudaStream_t stream) {
    if (0 == len) return;
    ...
    divisive_kernel<<<grid_size, block_size, 0, stream>>>(_d_keys, _d_vals, len, interval_, capacity_);
}

template <typename KeyType, typename ValType>
void Divisive<KeyType, ValType>::dump(void *d_keys, void *d_vals, 
    ...
    divisive_dump_kernel<<<grid_size, block_size, 0, stream>>>(_d_keys, _d_vals, d_dump_counter, 
