...
std::shared_ptr<RawManager> RawManager::Create(const std::shared_ptr<ResourcesManager>& resource_mgr) {
...
std::shared_ptr<States>& RawManager::get_optimizer_states(const std::string states_name) {
    ...
    if (optimizer_states_.end() == iter) 
        throw std::runtime_error(ErrorBase + "Cannot find optimizer states whose name is: " +
                                 states_name);
    return iter->second;
}
