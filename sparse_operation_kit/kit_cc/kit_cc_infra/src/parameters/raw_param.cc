    ...
    for (size_t dev_id = 0; dev_id < resource_mgr->get_local_gpu_count(); ++dev_id) {
        ...
        // construct hashtable
        {
            ...
            if (use_hashtable_) {
                ...
                hashtables_[dev_id] = IdentityHashTable<int64_t, size_t>::create(max_vocabulary_size_per_gpu_);
            }
        }
    } // for dev_id
...
void RawParam::dump_to_file(const std::string filepath) {
    ...
    if (0 == resource_mgr_->get_worker_id()) {
        ...
        const size_t total_max_vocabulary_size = get_max_vocabulary_size_per_gpu() 
...
void RawParam::restore_from_file(const std::string filepath) {
    ...
    if (!file_exist(key_filename))
        throw std::runtime_error(ErrorBase + key_filename + " doesn't exist. "
    ...
    if (!file_exist(values_filename))
        throw std::runtime_error(ErrorBase + values_filename + " doesn't exist. "
